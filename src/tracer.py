...
def median(values: list[float]) -> float:
    ...
    This lives here, and both `eval_ci` and the dashboard import it, because
    they did not used to agree: one took `sorted[n // 2]` and the other a
    nearest-rank percentile, so the same 12 traces produced `generate`
    medians of 9763.3 ms and 9559.2 ms. Two components of one project
    reporting different medians for identical data is a bug that looks like a
    measurement.
    """
    if not values:
        return 0.0
...
def stage_latencies(rows: list[dict] | None = None) -> dict[str, dict]:
    """Per-stage timing summary over a set of span rows.

    Returns `{stage: {n, median_ms, min_ms, max_ms}}`. The min/max matter here
    more than they usually would: the first request of a run pays for loading
    the encoder, the reranker and the model, so its span durations are an
    order of magnitude above the rest and would drag any mean.
    """
    rows = read_traces() if rows is None else rows
